...
<?php foreach ($oldalak as $url => $oldal): ?>
    <?php
        ...
        if (isset($_SESSION['user'])) {
            if ($oldal['menun'][1] == 1) $lathato = true;
        } else {
            if ($oldal['menun'][0] == 1) $lathato = true;
        }

        if ($lathato && $oldal['szoveg'] != ''):
            if ($url == 'pilota') {
                $link = 'index.php?url=pilota/list';
            } else {
                $link = ($url == '/') ? 'index.php' : 'index.php?' . $url;
            }
    ?>
        <a href="<?= $link ?>"><?= htmlspecialchars($oldal['szoveg']) ?></a>
    <?php endif; ?>
<?php endforeach; ?>
</nav>
<main>
